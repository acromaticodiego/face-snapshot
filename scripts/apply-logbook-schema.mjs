#!/usr/bin/env node
/**
 * Crea el schema y el rol del Logbook Service en una base de datos que
 * ya existe.
 *
 ...
 * proyecto en marcha nunca vería `03-logbook.sql`, y la alternativa
 * —`docker compose down -v`— borra los rostros enrolados, la jornada
 * de todo el mundo y la auditoría entera.
 *
 ...
 *
 *   cd services/logbook-service && npx prisma migrate deploy
 *
...
const password = env.LOGBOOK_SVC_DB_PASSWORD;

if (!password) {
  console.error(
    'Falta LOGBOOK_SVC_DB_PASSWORD en el .env.\n' +
...
const sql = `
SET custom.logbook_svc_password = '${password.replace(/'/g, "''")}';
\\i /docker-entrypoint-initdb.d/03-logbook.sql
`;

console.log(`Aplicando el schema logbook_svc en ${container}…`);
...
try {
  ...
  console.log('  cd services/logbook-service && npx prisma migrate deploy');
} catch (error) {
