...
/**
 * Cliente Prisma del Auth Service (schema `auth_svc`).
 *
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor() {
    ...
    if (!connectionString) {
      throw new Error(
        'AUTH_DATABASE_URL no está definida. Copia .env.example a .env.',
      );
    }
  ...
  async onModuleInit(): Promise<void> {
    await this.$connect();
    this.logger.log('Conectado a PostgreSQL (schema auth_svc)');
  }
